% CARE_pop_select() - pop_select modified by Fanny Grosselin
%				 Given an input EEG dataset structure, output a new EEG data structure 
%                retaining and/or excluding specified time/latency, data point, channel, 
%                and/or event range(s).
%
% Author of the modifications of pop_select: Fanny Grosselin, 2018
%
% Usage:
%   >> OUTEEG = CARE_pop_select(INEEG, 'key1', value1, 'key2', value2 ...);
%
% Graphic interface:
...
%   "Event range" - [edit box] RETAIN data epoch time locked to the indicated events name in the dataset.
%   "Event range" - [checkbox] EXCLUDE data epoch time locked to the specified events name in the dataset. 
%   "..." - [button] select events by name.
...
function [EEG, com] = CARE_pop_select( EEG, varargin);
...
if nargin < 2
    eventrange = [...
                   'eventl = findobj(''parent'',gcbf,''tag'',''eventr'');'...
                   'setappdata(0,''eventl'',eventl);'...
                   ...
                   '   for i = 1:length(tmpevent)'...
                        '    y = find(ismember(truelist,EEG.event(1,i).type));'...
                        '    if isempty(y)'...
                            '    truelist{end+1} = EEG.event(1,i).type;'...
                        '    end;'...
                   '   end;'...
                   '   setappdata(0,''truelist'',truelist);'...
                   '   clear i y;'...
                   '   pop_eventrange(truelist);' ...
				   'end;']; 

   geometry = { [1 1 1] [1 1 0.25 0.23 0.51] [1 1 0.25 0.23 0.51] [1 1 0.25 0.23 0.51] ...
           [1 1 0.25 0.23 0.51] [1 1 0.25 0.23 0.51] [1] [1 1 1]};
   uilist = { ...
         ...
         ...
         { 'Style', 'text', 'string', 'Event range (by name)' }, ...
         { 'Style', 'edit', 'string', '', 'tag', 'eventr' }, ...
         { }, { 'Style', 'checkbox', 'string', '    ' }, ...
         { 'style' 'pushbutton' 'string'  '...', 'callback' eventrange }, ...
         ...
   ...
   if ~isempty( results{9} )
       truelist = getappdata(0,'truelist');
       eventlist1 = getappdata(0,'eventli1');
       eventlist2 = getappdata(0,'eventli2');
       tmpevent = EEG.event;
       % Counter of apparition number of each selected event
       % ---------------------------------------------------
       count = zeros(1,2);
       if strcmp('END_OF_SIGNAL',eventlist1)
           count(1) = 1;
       else
           hi = find(ismember(truelist,eventlist1));
           for k = 1:length(tmpevent)
               if strcmp(tmpevent(1,k).type,truelist(hi))
                   count(1) = count(1)+1;
               end;
           end;
       end;
       if strcmp('END_OF_SIGNAL',eventlist2)
           count(2) = 1;
       else
           ki = find(ismember(truelist,eventlist2));
           for k = 1:length(tmpevent)
               if strcmp(tmpevent(1,k).type,truelist(ki))
                   count(2) = count(2)+1;
               end;
           end;
       end;
       % --------------------------------------------------
       if count(1) ~= count(2)
           errordlg2('It is not possible to perform this selection because the number of each marker chosen is different.');
           return;
       else
           if count(1)>1 || count(2)>1
               ...
               yo = 0;
               zo = 0;
               for x = 1:length(tmpevent)
                   if strcmp(eventlist1,tmpevent(1,x).type)
                       listresults3(1,yo+1) = tmpevent(1,x).latency;
                       yo = yo + 1;
                   end;
                   if strcmp(eventlist2,tmpevent(1,x).type)
                       listresults3(2,zo+1) = tmpevent(1,x).latency;
                       zo = zo + 1;
                   end;
               end;
               results{3} = ' ';
               for ji = 1:length(listresults3)
                   results{3} = [ results{3} num2str(listresults3(1,ji)) ' ' num2str(listresults3(2,ji)) '; '];
               end;
           else
               for i =1:length(tmpevent)
                   if (strcmp(tmpevent(1,i).type,eventlist1))
                       point1 = tmpevent(1,i).latency;
                   end;
                   if (strcmp('END_OF_SIGNAL',eventlist1))
                       point1 = EEG.pnts;
                   end;
                   if (strcmp(tmpevent(1,i).type,eventlist2))
                       point2 = tmpevent(1,i).latency;
                   end;
                   if (strcmp('END_OF_SIGNAL',eventlist2))
                       point2 = EEG.pnts;
                   end;
               end;
               if exist('point1') && exist('point2')
                   results{3} = sprintf('%s %s', num2str(point1), num2str(point2));   
               end;
           end;
       end;
       if ~results{10}, args = { args{:}, 'point', eval( [ '[' results{3} ']' ] ) };
       else            args = { args{:}, 'nopoint', eval( [ '[' results{3} ']' ] ) }; end;
   end;
