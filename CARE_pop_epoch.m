% CARE_pop_epoch() - pop_epoch modified by Fanny Grosselin
%				Convert a continuous EEG dataset to epoched data by extracting
...
%
% Author of the modifications of pop_epoch: Fanny Grosselin, 2018
%
% Usage:
%   >> OUTEEG = CARE_pop_epoch( EEG); % pop-up a data entry window
%   >> OUTEEG = CARE_pop_epoch( EEG, events, timelimits);
%   >> [OUTEEG, indices] = CARE_pop_epoch( EEG, typerange, timelimits,'key1', value1 ...);
%
% Graphic interface:
...
function [EEG, indices, com] = CARE_pop_epoch( EEG, events, lim, varargin );
...
if size(EEG.data,3) > 1
    epochlim = [num2str( round(EEG.xmin,1)) '  '  num2str(round(EEG.xmax,1))]; % Units in seconds as in GUI
%   epochlim = [num2str( round(EEG.xmin*1000,1)) '  'num2str(round(EEG.xmax*1000,1))]; % Units in miliseconds
else
    epochlim = '-1 2';
end
if exist('lim')
	epochlim = [num2str(lim(1)) ' ' num2str(lim(2))];
end;
OLDEEG = EEG;
...
%if nargin < 3
   ...
   uilist = { { 'style' 'text'       'string' 'Time-locking event type(s) ([]=all)' } ...
              { 'style' 'edit'       'string' events 'tag' 'events' } ...
   ...
   result = inputgui( geometry, uilist, 'pophelp(''CARE_pop_epoch'')', 'Extract data epochs - CARE_pop_epoch()');
...
if ~isempty(args)
   try, g = struct(args{:});
   catch, disp('CARE_pop_epoch(): wrong syntax in function arguments'); return; end;
else
    g = [];
end;
    ...
    if iscell(events)
		if isstr(EEG.event(1).type)
			...
			for index2 = 1:length( events )
				...
				if isempty( tmpevent ), error('CARE_pop_epoch(): string entered in a numeric field'); end;
				Ieventtmp = [ Ieventtmp find(tmpevent == [ tmpeventtype{:} ]) ];
			end;
		end;
    else
        error('CARE_pop_epoch(): multiple event types must be entered as {''a'', ''cell'', ''array''}'); return;
    end;
...
if isempty(alllatencies)
   error('CARE_pop_epoch(): empty epoch range (no epochs were found).'); return;
end;
fprintf('CARE_pop_epoch():%d epochs selected\n', length(alllatencies));
    ...
    if isfield(EEG.etc, 'amica') && ~isempty(EEG.etc.amica) && isfield(EEG.etc.amica, 'v_smooth') && ~isempty(EEG.etc.amica.v_smooth) && ~isfield(EEG.etc.amica,'prob_added')
        if isfield(EEG.etc.amica, 'num_models') && ~isempty(EEG.etc.amica.num_models)
            if size(EEG.data,2) == size(EEG.etc.amica.v_smooth,2) && size(EEG.data,3) == size(EEG.etc.amica.v_smooth,3) && size(EEG.etc.amica.v_smooth,1) == EEG.etc.amica.num_models
                ...
                %--------------------
                [EEG indices com] = CARE_pop_epoch(EEG,events,lim,args{:});
                %---------------------------------
...
switch lower( g.timeunit )
 ...
 case 'seconds',	[EEG.data tmptime indices epochevent]= epoch(EEG.data, alllatencies, lim, 'valuelim', g.valuelim, ...
	...
	otherwise, disp('CARE_pop_epoch(): invalid event time format'); beep; return;
end;
alllatencies = alllatencies(indices);
fprintf('CARE_pop_epoch():%d epochs generated\n', length(indices));


% update other fields
% -------------------
if lim(1) ~= tmptime(1) & lim(2)-1/EEG.srate ~= tmptime(2)
	fprintf('CARE_pop_epoch(): time limits have been adjusted to [%3.3f %3.3f] to fit data points limits\n', ...
...
% -------------------------
disp('CARE_pop_epoch(): checking epochs for data discontinuity');
...
% ---------------------
com = sprintf('%s = CARE_pop_epoch( %s, { ', inputname(1), inputname(1));
