% timefrequencyevoked() - function which calculate time-frequency maps of
%                         evoked activities and plot it.
...
function com = timefrequencyevoked( significant_line,unit, EEGnbchan, EEGtrials, EEGdata, resulparamclbk, nfft, EEGchanlocslabels, EEGchanlocs, listchannel, range_colorbar, EEGtimes, EEGsrate, EEGchaninfo)
...
for k = 1:length(truelistchannel)        
    ...
    fprintf('timefrequencyevoked : time frequency decomposition on channel %s\n', channelname{:});
	...
	EEG.CARE_rCortex(truelistchannel(k)).TFmap = Y;
end;
com = sprintf('timefrequencyevoked : time frequency decomposition\n');
...
if (length(truelistchannel) == 1)
    ...
    scrsz = get(0,'ScreenSize');
    h0 = axes('Position',[scrsz(3)/5, scrsz(4)/6, scrsz(3)/1.3, scrsz(4)/1.6]);
    myimage = imagesc(EEGtimes, freqvector*EEGsrate, Z{:});
	m=100;
	cm_plasma=plasma(m);
	colormap(h0,plasma);
    axis('xy'); % inversion of direction y
    title(sprintf('Time frequency evoked map on channel %s', char(EEGchanlocslabels(truelistchannel))),'edgecolor','red','linewidth',1);
    xlabel('Time (ms)');
    ylabel('Frequency (Hz)');
    if ~isnan(Z{:})
    ...
    savedata.algorithm = 'Evoked activities';
...
% to see time frequency map, he can choose channel(s) and push 'dispplay'
    ...
    tit = sprintf('Time frequency evoked map on channel');
    baseline = 'no';
    aux_plot = 0;
    marker_baseline = '';
    latpos = '';    
    ...
    signif_level = 0;
	M = {};
    evokedinducedtopoplot([],trueEEGchanlocs,trueEEGchanlocslabels, range_colorbar, EEGtimes, freqvector, EEGsrate, resulparamclbk, Z, M, trueunit,tit,aux_plot,significant_line,baseline,marker_baseline, latpos, listauxbase,len_base,'NaN','NaN',correction_baseline,signif_level,'Evoked activities','chaninfo', EEGchaninfo);  
end;
end
