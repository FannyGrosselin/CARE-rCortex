...
% Modified by Fanny Grosselin, 2018
...
if nargin < 2
	...
	%% Fanny Grosselin 2017 
	%  --------------------
	setappdata(0,'hdrfile',hdrfile);
    setappdata(0,'path',path);
    
    eventrange = [...
                   'hdrfile = getappdata(0,''hdrfile'');'...
                   ...
                   'clear i y path hdrfile MRK tmpevent hdr eventl;'...
                   'pop_eventrange(truelist);'];
				   
	uigeom = {[1 0.5 0.3] [1 0.5 0.3] [1 0.5 0.3]};
	uilist = {{ 'style' 'text' 'string' 'Interval (samples; e.g., [7 42]; default: all):'} ...
              { 'style' 'edit' 'string' ''} {}...
              { 'style' 'text' 'string' 'Channels (e.g., [1:2 4]; default: all):'} ...
              { 'style' 'edit' 'string' ''} {}...
              { 'style' 'text' 'string' 'Event range (see markers in the .vmrk file):'} ...
...
%% Fanny Grosselin 2017
%  --------------------
% Cut signal according events chosen
% ---------------------------------
if ~exist( 'event_chosen','var' )
    srange = [ 1 hdr.commoninfos.datapoints];
    EEG.pnts = hdr.commoninfos.datapoints;
else
    ...
    if count(1) ~= count(2)
        ...
        if count(1)>1 || count(2)>1
            ...
            for ji = 1:length(listresults3)
                event_chosen = [ event_chosen num2str(listresults3(1,ji)) ' ' num2str(listresults3(2,ji)) '; '];
            end;
        else
            for i =1:length(tmpevent)
            ...
            if exist('point1') && exist('point2')
                event_chosen = sprintf('%s %s', num2str(point1), num2str(point2));
            end;
        end;
    end;
    EEG = pop_select(EEG, 'point', eval( [ '[' event_chosen ']' ] ));
end;
% -----------------------------------------
