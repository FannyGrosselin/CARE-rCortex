% CARE_eegplot() - eegplot modified by Fanny Grosselin
%			  Scroll (horizontally and/or vertically) through multichannel data.
%             Allows vertical scrolling through channels and manual marking 
%             and unmarking of data stretches or epochs for rejection.
%
% Author of the modifications of eegplot: Fanny Grosselin, 2018
%
% Usage: 
%           >> CARE_eegplot(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> CARE_eegplot('noui', data, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
...
%    'baseline'   - [number array] Array containing two raws : first raw
%                   contains the start of baselines on the raw signal; second 
%                   raw contains the end of baselines on the raw signal.
%                   This keyword is used in case of use of "Wavelet
%                   analysis : evoked or induced activities".
%    'physio_mark'- [number vector] Vector containing the position of
%                   physiological event mark. This keyword is used in case of 
...
%                   If command is "baseline", you are able to remove, add 
%                   baselines and not reject  epochs. 
%                   If command is "physio_mark", you are able to remove,
%                   add event marks and not reject  epochs.
%    'butlabel'   - Reject button label or accept button baselines or markers in case of 'command'
%                   is 'baseline' or 'physio_mark'. {default: 'REJECT'}
...
%    'tag'        - [string] Matlab object tag to identify this CARE_eegplot() window (allows 
%                    keeping track of several simultaneous CARE_eegplot() windows). 
%    'children'   - [integer] Figure handle of a *dependent* CARE_eegplot() window. Scrolling
...
%                      CTRL key.
%	 'physio_name'- string that describes the type of events we are detecting.
% Outputs:
...
function [outvar1] = CARE_eegplot(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   try
       ...
       disp('CARE_eegplot() error: calling convention {''key'', value, ... } error'); return;
   end;	

  % push button: create/remove window
  % ---------------------------------
  defdowncom   = 'CARE_eegplot(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'CARE_eegplot(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'CARE_eegplot(''defupcom'',     gcbf);';
  defctrldowncom = 'CARE_eegplot(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.baseline;          catch, g.baseline = []; end; % FannyGrosselin
   try, g.physio_mark;       catch, g.physio_mark = []; end; % FannyGrosselin
   try, g.physio_name;       catch, g.physio_name = []; end; % FannyGrosselin
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' ...
               ...
               'maxeventstring' 'ploteventdur' 'butlabel' 'scale' 'events' 'data2' 'plotdata2' 'mocap' 'selectcommand' 'ctrlselectcommand' 'datastd' 'normed' 'envelope' 'baseline' 'physio_mark' 'physio_name'},;
      otherwise, error(['CARE_eegplot: unrecognized option: ''' gfields{index} '''' ]);
      end;
   end;
   ...
   if ~isempty(g.baseline) && size(g.baseline,1)~=2 % FannyGrosselin
        disp('Error: baseline must have array containing start (first raw), end (second raw) of baselines'); return;
   end;
   if ~isempty(g.physio_mark) && size(g.physio_mark,1)~=1 % FannyGrosselin
        disp('Error: physio_mark must be a vector containing position of physiological events'); return;
   end;
   
   % Baseline
   % -------- FannyGrosselin
   if ~isempty(g.baseline)
       g.accept_baseline = g.baseline; % at the beginning, baselines are baselines input
   end;
   
   % Physio_mark
   % ----------- FannyGrosselin
   if ~isempty(g.physio_mark)
       g.accept_physio_mark = g.physio_mark; % at the beginning, physio_mark are physio_mark input
   end;
   
   % Physio_name
   % ----------- FannyGrosselin
   if ~isempty(g.physio_name)
       if strcmp(g.physio_name,'resp_event')
           g.winlength = 20;
       elseif strcmp(g.physio_name,'cardiac_event')
	       g.winlength = 5;
       end;
   end;
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      ...
      CARE_eegplot('setelect', g.eloc_file, ax1);
  end;
...
u(22) = uicontrol('Parent',figh, ...
  ...
  if ~isempty(g.command) && ~strcmp(g.command,'baseline') && ~strcmp(g.command,'physio_mark')
  ...
  % Baseline
  % --------  FannyGrosselin
   acceptbaseline = ['g = get(gcbf, ''userdata'');' ... 
                    'EEG = evalin(''base'',''EEG'');'...
                    'minb = g.accept_baseline(1,:);'... 
                    'maxb = g.accept_baseline(2,:);'...
                    ... % find the nearest points time of baseline
                    ... % ----------------------------------------
                    'kal = 1;'...
                    'counter = 0;'...
                    'for is = EEG.pnts:EEG.pnts:EEG.pnts*EEG.trials'...
                        '    while (is-EEG.pnts >= int64(minb(kal)))'...
                            '    is = is-EEG.pnts;'...
                        '    end;'...
                        '    while (kal <= length(minb) && is-EEG.pnts < int64(minb(kal))  && int64(minb(kal)) <= is)'...
                    ...
                    'counter = 0;'...
                    'for is = EEG.pnts:EEG.pnts:EEG.pnts*EEG.trials'...
                        '    while (is-EEG.pnts >= int64(maxb(kal)))'...
                            '    is = is-EEG.pnts;'...
                        '    end;'...
                        '    while kal <= length(maxb) && is-EEG.pnts < int64(maxb(kal))  && int64(maxb(kal)) <= is'...
                    ...
                    'end;'...
                    'clear kal is si counter;'...
                    ... % Test if baseline is in the epoch and located before marker event (t = 0)
                    ... % ------------------------------------------------------------------------
                    'bad_base_minb = find(minb >= 0);'... 
                    'bad_base_maxb = find(maxb > 0);'... 
                    'bad_base = sort(unique([event_assoc_minb(bad_base_minb),event_assoc_maxb(bad_base_maxb)]));'... % find epoch number that have these bad baselines
                    'if ~isempty(bad_base)'...
                          '    bad_epoch = [];'...
                          '    for kl = 1:length(bad_base)'...
                             '    bad_epoch = [bad_epoch,sprintf(num2str(bad_base(kl))),'', ''];'...
                          '    end;'...
                          '    errordlg2(sprintf(''Warning : epoch(s) number %s has/have a baseline which is out of epoch limits or after marker event!\n You must modify baselines or modify epoch limits or remove corresponding epochs.'', bad_epoch),''Warning'');'...
                          '    disp(sprintf(''Warning : epoch(s) number %s has/have a baseline which is out of epoch limits or after marker event!'', bad_epoch));'...
                          '    remove_epoch = questdlg2(''Do you want to remove labeled trials with a bad baseline ?'',''Remove Epoch(s) Question'',''Cancel'', ''Yes'', ''No'', ''Yes'');'...
                          '    switch remove_epoch,'...
                              '    case ''Cancel'','...
                                  '    clear bad_epoch kl bad_base event_assoc_minb event_assoc_maxb minb maxb bad_base_minb bad_base_maxb remove_epoch;'...
                                  '    return;'...
                              '    case ''Yes'','...
                                  ... % Rejection of labeled trials 
                                  ... % ---------------------------
                                  '    number_trials = EEG.trials;'...
                                  '    rej = zeros(1,EEG.trials);'...
                                  '    for jou = 1:length(bad_base)'...
                                        '    rej(bad_base(jou)) = 1;'...
                                  '    end;'...
                                  '    clear jou;'...
                                  '    EEG = pop_rejepoch(EEG,rej,0);'... % reject epochs
                                  '    [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG, CURRENTSET);'...
                                  '    eegh(''[ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG, CURRENTSET);'');'...
                                  '    errordlg2(''Warning : do not forget to save dataset after analysis !'',''Warning'');'...
                                  '    if EEG.trials == number_trials'... % if the user has cancelled rejection
                                        '    clear number_trials bad_base bad_base_minb bad_base_maxb bad_epoch event_assoc_minb event_assoc_maxb kl maxb minb rej remove_epoch;'...
                                        '    return;'...
                                  '    else'... % if the user hasn't cancelled rejection
                                      '    clear number_trials;'...
                                      ... % Rejection of baselines on labeled trials which will be rejected
                                      ... % --------------------------------------------------------------
                                      '    num_rej_epoch = find(rej>0);'... % find the epoch numbers that we must remove
                                      '    for i = 1:length(num_rej_epoch)'...
                                            '    num_rej_start_base{i} = [find(ismember(event_assoc_minb,num_rej_epoch(i)))];'... % find the baseline numbers that there are contained in removed epochs
                                            '    num_rej_base = num_rej_start_base;'...
                                      '    end;'...
                                      '    clear i;'...
                                      ... % Print baseline numbers to remove
                                      '    base_rej = [];'...
                                      '    for ji = 1:length(num_rej_base)'...
                                          '    for ka = 1:length(num_rej_base{1,ji})'...
                                              '    base_rej = [base_rej,sprintf(num2str(num_rej_base{1,ji}(ka))),'', ''];'...
                                          '    end;'...
                                      '    end;'...
                                      '    clear ji ka;'...
                                      '    disp (sprintf(''Baseline number(s) %s has/have been rejected'',base_rej));'...
                                      ... % Remove baselines contained in removed epochs
                                      '    cra = 1;'...
                                      '    while cra <= size(g.accept_baseline,2) && ~isempty(num_rej_base)'...% shift of baseline
                                            '    if length(num_rej_base{1,1}) == 1 && cra == num_rej_base{1,1}'...
                                                '    g.accept_baseline(:,cra) = [];'...
                                                '    g.accept_baseline(:,cra:end) = g.accept_baseline(:,cra:end) - EEG.pnts;'... 
                                                '    num_rej_base(1) = [];'...
                                                '    if ~isempty(num_rej_base)'...
                                                    '    for ji = 1:length(num_rej_base)'...
                                                        '    for ka = 1:length(num_rej_base{1,ji})'...
                                                            '    num_rej_base{1,ji}(ka) = num_rej_base{1,ji}(ka)-1;'...
                                                        '    end;'...
                                                    '    end;'...
                                                '    end;'...
                                                '    clear ji ka;'...
                                                '    cra = 0;'...
                                            '    elseif length(num_rej_base{1,1}) > 1 && cra == num_rej_base{1,1}(1)'...
                                                    '    g.accept_baseline(:,cra) = [];'...
                                                    '    num_rej_base{1,1}(1) = [];'...
                                                    '    if ~isempty(num_rej_base)'...
                                                        '    for ji = 1:length(num_rej_base)'...
                                            ...
                                            '    cra = cra+ 1;'...
                                      '    end;'...         
                                      '    clear num_rej_epoch num_rej_base num_rej_start_base base_rej cra;'...
                                      ...
                                      '    clear rej bad_epoch kl event_assoc_minb event_assoc_maxb minb maxb bad_base_minb bad_base_maxb remove_epoch;'...
                                      '    errordlg2(''You must again check and accept baselines.'',''Warning'');'...
                                      '    set(gcbf,''UserData'', g);'...
                                      '    close(gcbf);'...
                                      '    CARE_eegplot (EEG.data(indice_chan,:,:),''srate'',EEG.srate,''title'',''Remove, correct, add or accept baseline on each epoch -- CARE_eegplot()'',''events'',EEG.event,''limits'',[EEG.xmin EEG.xmax]*1000,''butlabel'',''ACCEPT'',''command'',''baseline'',''baseline'', g.accept_baseline);'...
                                  '    end;'...
                              '    case ''No'','...
                                  '    clear bad_epoch kl bad_base event_assoc_minb event_assoc_maxb minb maxb bad_base_minb bad_base_maxb remove_epoch;'...
                                  '    return;'...
                    ...
                    'end;'...
                    'if isempty(bad_base)'...
                        ... % Test if each epoch has ONE baseline
                        ... % -----------------------------------
                        '    base_toomuch_minb = find(event_assoc_minb(1:length(event_assoc_minb)-1) == event_assoc_minb(2:length(event_assoc_minb)));'...
                        '    base_toomuch_maxb = find(event_assoc_maxb(1:length(event_assoc_maxb)-1) == event_assoc_maxb(2:length(event_assoc_maxb)));'...
                        '    base_any_epoch = [event_assoc_minb(base_toomuch_minb),event_assoc_maxb(base_toomuch_maxb)];'...
                        '    base_any_epoch = sort(unique([base_any_epoch,setdiff([1:EEG.trials],event_assoc_minb),setdiff([1:EEG.trials],event_assoc_maxb)]));'...% find epoch number that have these bad baselines
                        '    if ~isempty(base_any_epoch)'...
                           '    any_epoch = [];'...
                           '    for kl = 1:length(base_any_epoch)'...
                              '    any_epoch = [any_epoch,sprintf(num2str(base_any_epoch(kl))),'', ''];'...
                           '    end;'...
                           '    errordlg2(sprintf(''Warning : epoch(s) number %s has/have not or too much baseline(s) ! Please, check your baselines !'', any_epoch),''Warning'');'...
                           '    disp(sprintf(''Warning : epoch(s) number %s has/have not or too much baselines !'', any_epoch));'...
                           '    remove_epoch_bis = questdlg2(''Do you want to remove labeled trials with a bad baseline ?'',''Remove Epoch(s) Question'',''Cancel'', ''Yes'', ''No'', ''Yes'');'...
                           '    switch remove_epoch_bis,'...
                              '    case ''Cancel'','...
                                  '    clear remove_epoch_bis any_epoch base_any_epoch kl event_assoc_minb event_assoc_maxb base_toomuch_minb base_toomuch_maxb minb maxb bad_base bad_base_minb bad_base_maxb;'...
                                  '    return;'...
                              '    case ''Yes'','...
                                  ... % Rejection of labeled trials 
                                  ... % ---------------------------
                                  '    number_trials = EEG.trials;'...
                                  '    rej = zeros(1,EEG.trials);'...
                                  '    for jou = 1:length(base_any_epoch)'...
                                        '    rej(base_any_epoch(jou)) = 1;'...
                                  '    end;'...
                                  '    clear jou;'...
                                  '    EEG = pop_rejepoch(EEG,rej,0);'... % reject epochs
                                  '    [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG, CURRENTSET);'...
                                  ...
                                  '    errordlg2(''Warning : do not forget to save dataset after analysis !'',''Warning'');'...
                                  '    if EEG.trials == number_trials'... % if the user has cancelled rejection
                                        '    clear number_trials any_epoch base_any_epoch kl event_assoc_minb event_assoc_maxb rej remove_epoch base_toomuch_minb base_toomuch_maxb minb maxb bad_base bad_base_minb bad_base_maxb;'...
                                        '    return;'...
                                  '    else'... % if the user hasn't cancelled rejection
                                       '    clear number_trials;'...
                                       ... % Rejection of baselines on labeled trials which will be rejected
                                       ...
                                       ... % Remove baselines contained in removed epochs
                                       '    cra = 1;'...
                                       '    while cra <= size(g.accept_baseline,2) && ~isempty(num_rej_epoch)'...
                                            '    if length(num_rej_base{1,1}) == 1 && cra == num_rej_base{1,1}'...
                                                '    g.accept_baseline(:,cra) = [];'...
                                                '    g.accept_baseline(:,cra:end) = g.accept_baseline(:,cra:end) - EEG.pnts;'... 
                                                '    num_rej_base(1) = [];'...
                                                '    num_rej_epoch(1) = [];'...
                                                '    if ~isempty(num_rej_epoch), num_rej_epoch = num_rej_epoch - 1; end;'...
                                                '    if ~isempty(num_rej_base)'...
                                                    '    for ji = 1:length(num_rej_base)'...
                                                        '    for ka = 1:length(num_rej_base{1,ji})'...
                                                            '    num_rej_base{1,ji}(ka) = num_rej_base{1,ji}(ka)-1;'...
                                            ...
                                            '    elseif length(num_rej_base{1,1}) > 1 && cra == num_rej_base{1,1}(1)'...
                                                ...
                                                '    clear ji ka;'...
                                                '    cra = 0;'...
                                            '    elseif length(num_rej_base{1,1}) == 0 && cra == num_rej_epoch(1)'...
                                                '    g.accept_baseline(:,cra:end) = g.accept_baseline(:,cra:end) - EEG.pnts;'... 
                                                '    num_rej_base(1) = [];'...
                                                '    num_rej_epoch(1) = [];'...
                                                '    if ~isempty(num_rej_epoch), num_rej_epoch = num_rej_epoch - 1; end;'...
                                       ...
                                       '    clear num_rej_epoch num_rej_base num_rej_start_base base_rej cra;'...
                                       ...
                                       '    clear number_trials any_epoch base_any_epoch kl event_assoc_minb event_assoc_maxb rej base_toomuch_minb base_toomuch_maxb remove_epoch_bis minb maxb bad_base bad_base_minb bad_base_maxb;'...
                                       '    errordlg2(''You must again check and accept baselines.'',''Warning'');'...
                                       '    set(gcbf,''UserData'', g);'...
                                       '    close(gcbf);'...
                                       '    CARE_eegplot (EEG.data(indice_chan,:,:),''srate'',EEG.srate,''title'',''Remove, correct, add or accept baseline on each epoch -- CARE_eegplot()'',''events'',EEG.event,''limits'',[EEG.xmin EEG.xmax]*1000,''butlabel'',''ACCEPT'',''command'',''baseline'',''baseline'', g.accept_baseline);'...
                        ...
                        '    else'...
                           '    if g.children, delete(g.children); end;' ...
                           '    disp(''You have checked and accepted baselines. Now you can continue to display analyse.'');'...
                           '    errordlg2(''You have checked and accepted baselines. Now you can continue to display analyse.'',''Accepted baselines'');'...
                           '    ACCEPTBASELINE = g.accept_baseline;' ...% Save baselines which may be modified, added or removed
                           '    delete(gcbf);' ...
                           '    clear g base_any_epoch any_epoch event_assoc_minb event_assoc_maxb minb maxb bad_base bad_base_minb bad_base_maxb base_toomuch_minb base_toomuch_maxb;'...
                        '    end;'...
                   'end;'...
                   'clear bad_base;']; 
  if ~isempty(g.command) && strcmp(g.command,'baseline')
	  u(12) = uicontrol('Parent',figh, ...
						'Units', 'normalized', ...
						'Position',posbut(12,:), ...
						'Tag','Accept',...
						'string',g.butlabel, 'callback', acceptbaseline);
  end;
  
  % Physio_mark
  % -----------  FannyGrosselin
   accept_physio_mark = ['g = get(gcbf, ''userdata'');' ... 
                    '    EEG = evalin(''base'',''EEG'');'...
                    '    ACCEPT_PHYSIO_MARK = g.accept_physio_mark;' ...% Save accept_physio_mark which may be modified, added or removed
                    '    close(gcbf);'...
                    '    close gcf;'...
                    ...
                    ... % Save event in the EEGLAB structure
                    ... % ----------------------------------
                    '    event_name = inputdlg2({''Write the name you want to give for the detected events :''},''Name of events'',1,{g.physio_name});'...
                    '    for i = 1:length(ACCEPT_PHYSIO_MARK)'...
                        '    EEG.event(1,i).latency = ACCEPT_PHYSIO_MARK(i);'...
                        '    EEG.event(1,i).type = event_name{:};'...
                    '   end;'...
                    ... % Extract epochs
                    ... % --------------
                    'extr = questdlg2(''Do you want to extract epoch now ?'',''Extract epoch Question'',''No'', ''Yes'', ''Yes'');'...
                              'switch extr,'...
                                  '    case ''No'','...
                                  '    case ''Yes'','...
                                        '    distance_moy = [];'...
                                        '    for jk = 2 : length(ACCEPT_PHYSIO_MARK)'...
                                            '    distance_moy = [distance_moy; abs(ACCEPT_PHYSIO_MARK(jk) - ACCEPT_PHYSIO_MARK(jk-1))];'...
                                        '    end;'...
                                        '    quantile_physio_mark = quantile(distance_moy/EEG.srate,0.90);'...
                                        '    moy_physio_mark = sum(distance_moy/EEG.srate)/length(distance_moy);'...
                                        '    wh = warndlg({sprintf(''%s %f %s'',''The average distance between two markers is '',moy_physio_mark, '' seconds.'');sprintf(''%s %f %s'',''The 0.9 quantile between two markers is '', quantile_physio_mark,'' seconds.'');''Warning to the time limits when you will extrat epochs in order to avoid overlapping of trials. Also THE MINIMUM LENGTH OF AN EPOCH MUST BE AT LEAST 1S FOR CARE-rCORTEX WAVELET ANALYSIS.''});'...
										'    uiwait(wh);'...
										'    if moy_physio_mark>=1'...
											'    valLimMax = moy_physio_mark-0.5;'...
											'    EEG = CARE_pop_epoch(EEG,event_name{:},[-0.5 valLimMax]);'...
										'    else'...
											'    EEG = CARE_pop_epoch(EEG,event_name{:},[-0.5 0.5]);'...
										'    end;'...
                                        '    [ALLEEG EEG CURRENTSET] = pop_newset( ALLEEG, EEG, CURRENTSET);'...
                                        '    eeglab redraw;'...
                                        '    wh = warndlg({''Several baseline corrections are available in '';''Tools > CARE-rCortex analysis > Wavelet analysis: evoked or induced activities''});'...
										'    uiwait(wh);'...
							  'end;'...
                              ' clear quantile_physio_mark moy_physio_mark jk distance_moy extr event_name i g indice_chan qrs_i_raw']; 
                
  if ~isempty(g.command) && strcmp(g.command,'physio_mark')
	  u(12) = uicontrol('Parent',figh, ...
						'Units', 'normalized', ...
						'Position',posbut(12,:), ...
						'Tag','Accept',...
						'string',g.butlabel, 'callback', accept_physio_mark);
  end;
  
  if ~strcmp(g.command,'baseline') & ~strcmp(g.command,'physio_mark')
      u(13) = uicontrol('Parent',figh, ...
        ...
        'string',fastif(isempty(g.command),'CLOSE', 'CANCEL'), 'callback', ...
            [	'g = get(gcbf, ''userdata'');' ... 
                'if g.children, delete(g.children); end;' ...
                'close(gcbf);'] );
  elseif strcmp(g.command,'baseline') % Baseline -> FannyGrosselin
      cancelbaseline = [...
                  ... % answer to the user to cancel
                  'canc = questdlg2(''What do you want to do ?'',''Cancel Baseline Question'',''Cancel'', ''Remove all modifications'', ''Cancel latest modifications'', ''Cancel latest modifications'');'...
                  'switch canc,'...
                      '    case ''Cancel'','...
                      '    case ''Cancel latest modifications'','...
                          '    g = get(gcbf, ''userdata'');' ...
                          '    if exist(''ACCEPTBASELINE'')'...
                                '    ACCEPTBASELINE = g.baseline;'... % FannyGrosselin : recover baselines input
                          '    end;'...
                          '    if g.children, delete(g.children); end;' ...
                          '    clear g;'...
                          '    close(gcbf);'...
                          '    disp(''You canceled latest modifications.'');'...
                      '    case ''Remove all modifications'','...
                          '    g = get(gcbf, ''userdata'');' ...
                          '    clear ACCEPTBASELINE;'... % FannyGrosselin : recover first baselines without modifications
                          '    clear g;' ...
                          '    close(gcbf);'...
                          '    disp(''You have removed all modifications except for epoch rejection.'');'...
                  'end;'...
                  'clear canc;']; % switch
      u(13) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position',posbut(13,:), ...
        'string','CANCEL', 'callback', cancelbaseline);
  elseif strcmp(g.command, 'physio_mark') % Physio_mark -> FannyGrosselin
      cancelphysiomark = [...
          ... % answer to the user to cancel
          'canc = questdlg2(''Are you sure to cancel ?'',''Cancel Physio mark Question'',''Yes'', ''No'', ''No'');'...
          'switch canc,'...
          '    case ''No'','...
          '    case ''Yes'','...
                '    g = get(gcbf, ''userdata'');' ...
                ...
                '    close(gcbf);'...
                '    close(gcf);'...
                '    disp(''You canceled physiological marker detection.'');'...
          'end;'...
          'clear canc;']; % switch
      u(13) = uicontrol('Parent',figh, ...
          'Units', 'normalized', ...
          'Position',posbut(13,:), ...
          'string','CANCEL', 'callback', cancelphysiomark);
  end;
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Event types', 'callback', 'CARE_eegplot(''drawlegend'', gcbf)');
  end;
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'CARE_eegplot(''noui'');');
  uimenu('Parent',m(7),'Label','Accept and close', 'Callback', acceptcommand ,'enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'));
  uimenu('Parent',m(7),'Label','Cancel and close', 'Callback','delete(gcbf)','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'));
  % Portrait %%%%%%%%+raj
  ...
  m(11) = uimenu('Parent',m(1),'Label','Data select/mark', 'tag', 'displaywin', ...
            'userdata', { 1, [0.8 1 0.8], 0, fastif( g.trialstag(1) == -1, 0, 1)},'enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'));
  ...
  % X grid %%%%%%%%%%%%
  m(3) = uimenu('Parent',m(1),'Label','Grid','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'));
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'CARE_eegplot(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','CARE_eegplot(''title'')')
  ...
  uimenu('Parent',m(1),'Label',fastif(g.envelope == 0, ...
         'Stack channels','Spread channels'), 'Callback',cb, 'Tag', 'Envelope_menu','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'))
  ...
  cb_normalize = ['g = get(gcbf,''userdata'');if g.normed, disp(''Denormalizing...''); else, disp(''Normalizing...''); end;'...
    ...
    'disp(''Done.'')'];
  uimenu('Parent',m(1),'Label',fastif(g.envelope == 0, ...
         'Normalize channels','Denormalize channels'), 'Callback',cb_normalize, 'Tag', 'Normalize_menu','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'))
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','CARE_eegplot(''window'')')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','CARE_eegplot(''winelec'')','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'))
  
  % Electrodes %%%%%%%%
  m(6) = uimenu('Parent',m(2),'Label','Channel labels','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'));
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','CARE_eegplot(''loadelect'');')
  
  % Zooms %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Zoom off/on','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'));
  commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); CARE_eegplot(''''zoom'''', gcbf, 1);'' ]);' ...
  ...
  uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback', ...
     ...
     'clear tmpg;' ]);
  uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''CARE_eegplot'');');

  % Events %%%%%%%%
  zm = uimenu('Parent',m(2),'Label','Events','enable', fastif((isempty(g.baseline) & isempty(g.physio_mark)), 'on', 'off'));
  complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''on'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; CARE_eegplot(''drawp'', 0);'];
  comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''off'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; CARE_eegplot(''drawp'', 0);'];
  comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                'tmpg.plotevent = ''on'';' ...
                'set(gcbf, ''userdata'', tmpg); clear tmpg; CARE_eegplot(''emaxstring'');']; % JavierLC      
  comeventleg  = [ 'CARE_eegplot(''drawlegend'', gcbf);'];
...
%  set(figh, 'busyaction', 'cancel');
  ...
  CARE_eegplot('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  CARE_eegplot('scaleeye', 'off', gcf);
  end;
  
  CARE_eegplot('drawp', 0);
  if g.dispchans ~= g.chans
  	   CARE_eegplot('zoom', gcf);
  end;  
  CARE_eegplot('scaleeye', [], gcf);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  case 'drawp' % Redraw EEG and change position
    % this test help to couple CARE_eegplot windows
    if exist('p3')
    	figh = p3;
    	figure(p3);
    else	
    ...
    % channels in the CARE_eegplot figure window.
    for i = 1:g.chans
...
%        keyboard;  
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		CARE_eegplot( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
     ...
     if ~isempty(g.baseline)
         EEG = evalin('base','EEG');
         if 0 >= g.accept_baseline(1,1) % if the first baseline is before the first epoch, remove it
             g.accept_baseline(:,1) = [];
             disp('The first baseline has been removed because it overstepped limits.');
         end;
         if g.accept_baseline(2,end) > length(EEG.times)*EEG.trials % if the last baseline is after the last epoch, remove it
             g.accept_baseline(:,end) = [];
             disp('The last baseline has been removed because it overstepped limits.');
         end;
         axes(ax1);
         hold on;           
         start_ep = g.time * multiplier + 1;
         end_ep = (g.time + g.winlength) * multiplier;
         indic_base_minb = find(g.accept_baseline(1,:) >= start_ep);
         indic_base_maxb = find(g.accept_baseline(2,:) <= end_ep);
         indic_base = intersect(indic_base_minb,indic_base_maxb);
         K = multiplier*g.time;
         for ka = 1:length(indic_base)
             ix = [(g.accept_baseline(1,indic_base(ka)) - K) (g.accept_baseline(1,indic_base(ka)) - K) (g.accept_baseline(2,indic_base(ka)) - K) (g.accept_baseline(2,indic_base(ka)) - K)];
         ...
         set(gcf, 'userdata', g);
         clear ka ix igrec zed start_ep end_ep indic_base_minb indic_base_maxb indic_base;
     end
     ...
     if ~isempty(g.physio_mark)
         EEG = evalin('base','EEG');
         if 0 >= g.accept_physio_mark(1) % if the first physio_mark is before the first epoch, remove it
             g.accept_physio_mark(1) = [];
             disp('The first physio mark has been removed because it overstepped limits.');
         end;
         if g.accept_physio_mark(end) > length(EEG.times)*EEG.trials % if the last physio_mark is after the last epoch, remove it
             g.accept_physio_mark(end) = [];
             disp('The last physio mark has been removed because it overstepped limits.');
         end;
         axes(ax1);
         hold on; 
         ...
         indic_physio_mark = find(g.accept_physio_mark(:) >= lowlim & g.accept_physio_mark(:) <= highlim);
         for index = 1:length(indic_physio_mark)
             % draw latency line
             % -----------------
             tmplatphysio = g.accept_physio_mark(indic_physio_mark(index))-lowlim-1;
             tmpphysio   = plot([ tmplatphysio tmplatphysio ], ylim, 'color', 'm', ...
                 'linestyle', '-');
         end;
     end
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   CARE_eegplot('updateslider', fig);
   CARE_eegplot('drawp',0);	
   CARE_eegplot('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	CARE_eegplot('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'scaleeye'
	...
	text(double(.5),double((YLim(2)-YLim(1))/23+Yl(1)),num2str(g.spacing,4),...
	...
	if ~isempty(SPACING_UNITS_STRING)
         text(double(.5),double(-YLim(2)/23+Yl(4)),SPACING_UNITS_STRING,...
			 'HorizontalAlignment','center','FontSize',10, 'tag', 'thescale')
	end
	text(double(.5),double((YLim(2)-YLim(1))/10+Yl(1)),'Scale',...
		 'HorizontalAlignment','center','FontSize',10, 'tag', 'thescale')
    set(eyeaxes, 'tag', 'eyeaxes');
    
  case 'noui'
      if ~isempty(varargin)
          CARE_eegplot( varargin{:} ); fig = gcf;
      else 
          fig = findobj('tag', 'CARE_EEGPLOT');
      end;
   ...
   case 'zoom' % if zoom
      ...
      % ------------------------------
      if exist('p2') == 1
          set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); CARE_eegplot(''zoom'', gcbf, 1);' ]);
  ...
  case 'defupcom'
      fig = varargin{1};
      g = get(fig,'UserData');
      if isempty(g.baseline) & isempty(g.physio_mark)
          ax1 = findobj('tag','backeeg','parent',fig);
          g.incallback = 0;
  ...
  % ---------------------------------
      case 'defdowncom'
          ...
          ax1 = findobj('tag','backeeg','parent',fig);
          tmppos = get(ax1, 'currentpoint');
          if strcmp(get(fig, 'SelectionType'),'normal') && isempty(g.baseline) && isempty(g.physio_mark);
              ...
              if g.incallback ~= 1 % interception of nestest calls
                  ...
                  if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                      ...
                      set(fig,'UserData', g);
                      CARE_eegplot('drawp', 0);  % redraw background
                  end;
              end;
          % Baseline
          % --------  FannyGrosselin
          elseif ~isempty(g.baseline)
              true_baselinepos = tmppos(1) + g.time*g.trialstag;
              search_truebaselinepos1 = find(g.accept_baseline(1,:) <= true_baselinepos);
              search_truebaselinepos2 = find(g.accept_baseline(2,:) >= true_baselinepos);
              search_truebaselinepos = intersect(search_truebaselinepos1,search_truebaselinepos2);
              if isempty(search_truebaselinepos) % if the point selected is not a baseline
                  leng = g.accept_baseline(2,1) - g.accept_baseline(1,1); % calcul of baseline length
                  % plot the futur baseline in green
                  K = g.trialstag*g.time;
                  ix = [tmppos(1) tmppos(1) (tmppos(1) + leng) (tmppos(1) + leng)];
                  igrec = [min(ylim) max(ylim) max(ylim) min(ylim)];
                  zed = [0 0 0 0];
                  patch(ix,igrec,zed,'g');
                  alpha(0.5);
                  clear K ix igrec zed;
                  % answer to the user to add baseline
                  Addbaseline = questdlg2('Do you want to add the green baseline here?', ...
                      'Control Baseline Question', ...
                      'Cancel', 'Yes', 'No', 'Yes');
                  switch Addbaseline,
                      case 'Cancel',
                      case 'Yes', % add a baseline
                          DECAL = g.accept_baseline; % add an empty colon to add baseline
                          g.accept_baseline = [g.accept_baseline, [NaN; NaN]]; % add an empty colon to add baseline
                          for si = 1:length(g.accept_baseline) % add the new baseline to the true position
                              if ~isempty(search_truebaselinepos1) && ~isempty(search_truebaselinepos2)
                                  if si <= search_truebaselinepos1(end) 
                                      g.accept_baseline(1,si) = DECAL(1,si);
                                      g.accept_baseline(2,si) = DECAL(2,si);
                                  elseif si == search_truebaselinepos1(end) + 1
                                      g.accept_baseline(1,si) = round(true_baselinepos);
                                      g.accept_baseline(2,si) = round(true_baselinepos + leng);
                                  else
                                      g.accept_baseline(1,si) = DECAL(1,si-1);
                                      g.accept_baseline(2,si) = DECAL(2,si-1);
                                  end;
                              elseif isempty(search_truebaselinepos1) % if new baseline is before first baseline
                                  if si == 1
                                      g.accept_baseline(1,si) = round(true_baselinepos);
                                      g.accept_baseline(2,si) = round(true_baselinepos + leng);
                                  else
                              ...
                              elseif isempty(search_truebaselinepos2) % if new baseline is after last baseline
                                  if si == length(g.accept_baseline)
                                      g.accept_baseline(1,si) = round(true_baselinepos);
                                      g.accept_baseline(2,si) = round(true_baselinepos + leng);
                                  else
                                      g.accept_baseline(1,si) = DECAL(1,si);
                          ...
                          disp('You added a baseline.');
                      case 'No',
                  end % switch
              else % if a baseline is selected
                  % plot the selected baseline in red
                  K = g.trialstag*g.time;
                  ix = [(g.accept_baseline(1,search_truebaselinepos) - K) (g.accept_baseline(1,search_truebaselinepos) - K) (g.accept_baseline(2,search_truebaselinepos) - K) (g.accept_baseline(2,search_truebaselinepos) - K)];
                  igrec = [min(ylim) max(ylim) max(ylim) min(ylim)];
                  zed = [0 0 0 0];
                  patch(ix,igrec,zed,'r');
                  alpha(0.5);
                  clear K ix igrec zed;
                  % answer to the user to remove baseline
                  Removebaseline = questdlg2('Do you want to remove this selected baseline ?', ...
                      'Control Baseline Question', ...
                      'Cancel', 'Yes', 'No', 'Yes');
                  switch Removebaseline,
                      case 'Cancel',
                      case 'Yes', % remove baseline
                          g.accept_baseline(:,search_truebaselinepos) = [];
                          disp('You removed a baseline.');
                      case 'No',
                  end % switch
              end;
              set(fig,'UserData', g);
          ...
          % physio_mark
          % -----------  FannyGrosselin
          elseif ~isempty(g.physio_mark)
              lowlim = round(g.time*g.srate+1);
              true_physio_mark_pos = tmppos(1) + lowlim;
              % --------------------------------------------------
              % Calcul of the averaged length between two markers
              moy_len_physio_mark = 0;
              for phi = 2:length(g.accept_physio_mark)
                  moy_len_physio_mark = moy_len_physio_mark + (g.accept_physio_mark(phi) - g.accept_physio_mark(phi-1));
              end;
              moy_len_physio_mark = moy_len_physio_mark/length(g.accept_physio_mark - 1);			  
              search_true_physio_mark_pos1 = find(g.accept_physio_mark(:) <= true_physio_mark_pos + 0.05*moy_len_physio_mark);
              search_true_physio_mark_pos2 = find(g.accept_physio_mark(:) >= true_physio_mark_pos - 0.05*moy_len_physio_mark);
              search_true_physio_mark_pos = intersect(search_true_physio_mark_pos1,search_true_physio_mark_pos2);
              if isempty(search_true_physio_mark_pos) % if the point selected is not around a physio_mark
                  tmphphysio_added   = plot([ tmppos(1) tmppos(1) ], ylim, 'color', 'g', ...
                 'linestyle', '-');
                  % answer to the user to add physio mark
                  Addphysiomark = questdlg2('Do you want to add the green physio mark here?', ...
                      'Control Physio mark Question', ...
                      'Cancel', 'Yes', 'No', 'Yes');
                  switch Addphysiomark,
                      case 'Cancel',
                      case 'Yes', % add a physio mark
                          DECAL = g.accept_physio_mark; % add an empty colon to add physio mark
                          g.accept_physio_mark = [g.accept_physio_mark, NaN]; % add an empty colon to add physio mark
                          for si = 1:length(g.accept_physio_mark) % add the new physio mark to the true position
                              if ~isempty(search_true_physio_mark_pos1) && ~isempty(search_true_physio_mark_pos2)
                                  if si <= search_true_physio_mark_pos1(end) 
                                      g.accept_physio_mark(si) = DECAL(si);
                                  elseif si == search_true_physio_mark_pos1(end) + 1
                                      g.accept_physio_mark(si) = round(true_physio_mark_pos);
                                  else
                                      g.accept_physio_mark(si) = DECAL(si-1);
                                  end;
                              elseif isempty(search_true_physio_mark_pos1) % if new physio mark is before first marker
                                  if si == 1
                                      g.accept_physio_mark(si) = round(true_physio_mark_pos);
                                  else
                                      g.accept_physio_mark(si) = DECAL(si-1);
                                  end;
                              elseif isempty(search_true_physio_mark_pos2) % if new physio mark is after last marker
                                  if si == length(g.accept_physio_mark)
                                      g.accept_physio_mark(si) = round(true_physio_mark_pos);
                                  else
                                      g.accept_physio_mark(si) = DECAL(si);
                                  end;
                          ...
                          disp('You added a marker.');
                      case 'No',
                  end % switch
              else % if a marker is selected
                  true_selected_marker = search_true_physio_mark_pos(1);
                  if length(search_true_physio_mark_pos) > 1 
                      cal = abs(g.accept_physio_mark(search_true_physio_mark_pos(1))-true_physio_mark_pos);
                      for jo  = 2:length(search_true_physio_mark_pos)
                          cal2 = abs(g.accept_physio_mark(search_true_physio_mark_pos(jo))-true_physio_mark_pos);
                          if cal2 < cal1
                              cal = cal2;
                              true_selected_marker = search_true_physio_mark_pos(jo);
                          end;
                      end;
                  end;
                  % plot the selected physio_mark in red
                  plot([ g.accept_physio_mark(true_selected_marker)-lowlim-1 g.accept_physio_mark(true_selected_marker)-lowlim-1], ylim, 'color', 'r', ...
                 'linestyle', '-');
                  % answer to the user to remove physio_mark
                  Removephysio_mark = questdlg2('Do you want to remove this selected marker ?', ...
                      'Control Marker Question', ...
                      'Cancel', 'Yes', 'No', 'Yes');
                  switch Removephysio_mark,
                      case 'Cancel',
                      case 'Yes', % remove physio_mark
                          g.accept_physio_mark(true_selected_marker) = [];
                          disp('You removed a marker.');
                      case 'No',
                  end % switch
              end;
              set(fig,'UserData', g);
      ...
      otherwise
          error(['Error - invalid CARE_eegplot() parameter: ',data])
  end
end
