% timefrequencyevokedBase() - function which calculate time-frequency maps of
%                             evoked activities normalized with baseline and plot it.
...
%                       
% Usage:
%   >> com = timefrequencyevokedBase(marker_baseline, latpos, listauxbase, significant_line, unit, aux_plot,EEGnbchan, EEGtrials, EEGdata, resulparamclbk, nfft, EEGchanlocslabels, EEGchanlocs, listchannel, range_colorbar, EEGtimes, EEGsrate, EEGchaninfo, len_base, MINB, MAXB, minb, maxb, correction_baseline, signif_level)
%
% Inputs:
...
%   thanks to minb and maxb and a plot of average of the selected channel is 
%   also plotted.
...
function com = timefrequencyevokedBase(marker_baseline, latpos, listauxbase, significant_line, unit, aux_plot,EEGnbchan, EEGtrials, EEGdata, resulparamclbk, nfft, EEGchanlocslabels, EEGchanlocs, listchannel, range_colorbar, EEGtimes, EEGsrate, EEGchaninfo, len_base, MINB, MAXB, minb, maxb, correction_baseline, signif_level)
...
for k = 1:length(truelistchannel)  
	fprintf('Computation for the %s th channel\n', num2str(k));
    A = sum(EEGdata(truelistchannel(k),:,epochChosen),3);
    A = A./length(epochChosen);
    [Y,freqvector] = waveletTransformFourierPAD(resulparamclbk(1).valparam,resulparamclbk(2).valparam,resulparamclbk(3).valparam,nfft,A);
	...
	if signif_level == 0 % no need to see the significant parts of the time-frequency map
		...
		% ---------------------------------------
		disp('Normalization of data with baseline, please wait...');
		% Calcul of baseline
		B = []; % initialization of baseline vector
		baseline_len = MAXB(1) - MINB(1);
		baseline_centre = MINB + baseline_len/2;
		baseline_centre = round(median(baseline_centre));
		B = Y(:,round(baseline_centre - baseline_len/2):round(baseline_centre + baseline_len/2)); % Construction of baseline vector to compute std
		mB = mean(B,2);
		stdB = std(B,0,2);
		if strcmp(correction_baseline,'z-score')
			Z{k} = bsxfun(@rdivide, (Y-mB), stdB); % without unit       %Z{k} = (Y(t,:)-mean(B))./std(B,1); 
		elseif strcmp(correction_baseline,'absolute')
			Z{k} = Y-mB; %unit^2/Hz      %Z{k} = Y(t,:)-mean(B);
		elseif strcmp(correction_baseline,'relative')
			Z{k} = bsxfun(@rdivide, Y, mB).*100; % percent     % Z{k} = Y(t,:)./mean(B).*100;
		elseif strcmp(correction_baseline,'dB')
			Z{k} = log10(bsxfun(@rdivide, Y, mB)).*10; %dB    %Z{k} = 10.*log10(Y(t,:)./mean(B))
		end;
		...
		ovlp = 0;
		if NB >= 4
			R = floor((Np-NB*0.75)/(NB*(1-0.75))); % number of permutation for the significance of the TF map related to the choice of the baseline 
								% (we want to cover all the points of the TF map with an overlap between each permuation of 75%)
			ovlp = 0.75;
		else % If NB < 4 data points, we don't overlap the baselines
			R = floor(Np/NB);
		end;
		...
		for i=1:R
			...
			if ovlp == 0
				if i == 1
					...
					BL_interv_ini = (i-1)*NB;
				end;
			else
				if i == 1
					BL_interv_ini = 1;
				else
					BL_interv_ini = round((i-1)*NB/4);
				end;
			end;
			BL = Y(:,BL_interv_ini:BL_interv_ini+NB-1);
			BL_mean  = nanmean(BL,2);
			
			% define another evoked TF map with half of the trials over the selected trials
			ind_tr = shuffle([1:length(epochChosen)]);
			randEpochChosen = epochChosen(ind_tr(1:round(length(epochChosen)/2)));
			rA = sum(EEGdata(truelistchannel(k),:,randEpochChosen),3);
			rA = rA./length(randEpochChosen);
			[rY,~] = waveletTransformFourierPAD(resulparamclbk(1).valparam,resulparamclbk(2).valparam,resulparamclbk(3).valparam,nfft,rA);
			rY = (sqrt((rY) .* conj(rY))).^2; % faster than abs(Y).^2;
			
			% normalize the permuted TF map
			permA = log10(bsxfun(@rdivide,rY,BL_mean)).*10;
		...
		mask=bsxfun(@lt,YdB,tails(:,1))| bsxfun(@gt,YdB,tails(:,2));
        % ----------------------------
        
        % % ---------------------------------------------------
        % % APPLY THE MASK TO THE NORMALIZED TIME-FREQUENCY MAP
        % % ---------------------------------------------------
        % Z{k} = Ynorm.*mask;
		Z{k} = Ynorm;
		M{k} = mask;
    ...
    EEG.CARE_rCortex(truelistchannel(k)).TFmap = Ynorm;
	
	channelname = trueEEGchanlocslabels(k);
    fprintf('timefrequencyevokedBase : time frequency decomposition on channel %s\n', channelname{:});
...
com = sprintf('timefrequencyevokedBase : time frequency decomposition\n');
...
if (length(truelistchannel) == 1)
    if aux_plot == 0
        ...
        title(sprintf('Time frequency evoked map (corrected with %s baseline) on channel %s', correction_baseline,char(EEGchanlocslabels(truelistchannel))),'edgecolor','red','linewidth',1);
        xlabel('Time (ms)');
        ylabel('Frequency (Hz)');
        if ~isnan(Z{:})
        ...
        savedata.algorithm = 'Evoked activities (corrected with baseline)';
        ...
        A = sum(EEG.data(truelistchannel,:,epochChosen)- mean(EEG.data(truelistchannel,:,epochChosen)),3);
        A = A./(length(epochChosen));
    
        plot(EEG.times,A);
        axis('xy'); % inversion of direction y
        xlabel('Times (ms)');
        ylabel(sprintf('%s %s','Intensity (',char(unit{truelistchannel}),')'));
        title(sprintf('Graph of %s (average based on %s trials)',char(EEGchanlocslabels(truelistchannel)),num2str(length(epochChosen))));
        c = colorbar;
        set(c,'visible','off');
        hold on;
